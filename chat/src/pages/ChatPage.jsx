import React, { useMemo, useState, useRef, useEffect, useLayoutEffect, Fragment } from "react";
import { Link } from "react-router-dom";
import { motion, AnimatePresence } from "framer-motion";
import {
  Search, Users, MessageSquare, ChevronLeft, ChevronRight, X,
  Menu, User, Settings, ArrowDownRight, ArrowUpRight, GripHorizontal, GripVertical, Home
} from "lucide-react";
import { ForceGraphView, HierarchyGraphView, RadialGraphView } from "../GraphViews.jsx";
...
// Helper to recursively expand nodes up to a depth
function expandNodeRecursively(nodeId, depth, maxDepth, expandedSet = new Set()) {
  if (depth > maxDepth) return expandedSet;
  ...
  if (depth < maxDepth) {
    ...
    for (const relatedId of related) {
      expandNodeRecursively(relatedId, depth + 1, maxDepth, expandedSet);
    }
  }
...
export default function App() {
  ...
  const [navigationPath, setNavigationPath] = useState([`contact:${CONTACTS[0].id}`]); // Breadcrumb trail
  const [graphLayout, setGraphLayout] = useState('motion'); // 'motion', 'graph', 'hierarchy', 'radial'
  ...
  const [graphSettings, setGraphSettings] = useState({
    ...
    warmupTicks: 100,
    velocityDecay: 0.4,
    enableForces: true, // Toggle for force simulation
  });
  ...
  // Calculate chat width constraints
  const calculateChatWidth = () => {
    const navWidth = navOpen ? 280 : 0;
    const availableWidth = window.innerWidth - navWidth - detailsWidth;
  ...
  useEffect(() => {
    ...
    setNavigationPath([contactNodeId]); // Reset breadcrumb trail
  }, [activeContactId]);

  // Enforce chat width constraints on window resize
  useEffect(() => {
    const handleResize = () => {
      const navWidth = navOpen ? 280 : 0;
      ...
      const currentChatWidth = window.innerWidth - navWidth - detailsWidth;

      // If chat is too wide, increase details width
      if (currentChatWidth > maxChatWidth) {
        const newDetailsWidth = window.innerWidth - navWidth - maxChatWidth;
        setDetailsWidth(Math.max(minDetailsWidth, newDetailsWidth));
      }
      // If chat is too narrow, decrease details width
      else if (currentChatWidth < minChatWidth) {
        const newDetailsWidth = window.innerWidth - navWidth - minChatWidth;
  ...
  return (
    <div className="h-14 border-b border-zinc-800 flex items-center px-4 gap-3 bg-zinc-900/50 backdrop-blur-sm">
      ...
      <div className="ml-auto flex items-center gap-3">
        <Link
          to="/"
          className="p-2 rounded-lg hover:bg-zinc-800 transition flex items-center gap-2"
...
function VerticalResizeHandle({ detailsWidth, setDetailsWidth, navOpen }) {
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e) => {
      ...
      const currentNavWidth = navOpen ? 280 : 0;
      const minChatWidth = 200; // minimum chat width
      const maxChatWidth = 800; // maximum chat width
      const minDetailsWidth = 300; // minimum details width

      const newDetailsWidth = window.innerWidth - e.clientX;

      // Calculate constraints
      const maxDetailsWidth = window.innerWidth - currentNavWidth - minChatWidth;
      const minDetailsWidthBasedOnMaxChat = window.innerWidth - currentNavWidth - maxChatWidth;

      // Clamp details width
      const clampedWidth = Math.max(
        minDetailsWidth,
        Math.min(maxDetailsWidth, newDetailsWidth)
      );

      // Also ensure chat doesn't exceed 800px
      const finalWidth = Math.max(clampedWidth, minDetailsWidthBasedOnMaxChat);
...
function DetailsColumn({
  ...
  maxDepth, setMaxDepth, graphSettings, setGraphSettings,
  navigationPath, setNavigationPath, graphLayout, setGraphLayout, setStatus
}) {
  ...
  // Apply force settings whenever they change
  useEffect(() => {
    ...
    try {
      if (graphSettings.enableForces) {
        const chargeForce = fg.d3Force('charge');
        if (chargeForce) chargeForce.strength(graphSettings.chargeStrength);

        const linkForce = fg.d3Force('link');
        if (linkForce) linkForce.distance(graphSettings.linkDistance);

        const centerForce = fg.d3Force('center');
        if (centerForce) centerForce.strength(graphSettings.centerStrength);

        // Reheat simulation to apply changes
        fg.d3ReheatSimulation();
      } else {
        // Disable forces by setting to null
        fg.d3Force('charge', null);
        fg.d3Force('link', null);
        fg.d3Force('center', null);
      }
    } catch (error) {
      console.warn('Error applying force settings:', error);
    }
  }, [graphSettings.chargeStrength, graphSettings.linkDistance, graphSettings.centerStrength, graphSettings.enableForces]);

  const handleNodeClick = (node) => {
    setSelectedNode(node);
    setFocusNode(node.id); // Change focus to clicked node

    // Add to navigation path if not already the last item
    if (navigationPath[navigationPath.length - 1] !== node.id) {
  ...
  const handleBreadcrumbClick = (nodeId, index) => {
    setFocusNode(nodeId);
    ...
    if (nodeInfo) {
      ...
      setStatus(`Navigated back to ${nodeInfo.label}`);
    }
  };
  ...
  const handleRefocus = () => {
    ...
    // Refocus: center on selected node and recursively expand up to maxDepth
    setFocusNode(selectedNode.id);
    const newExpanded = expandNodeRecursively(selectedNode.id, 0, maxDepth);
    setExpandedNodes(newExpanded);
    setStatus(`Refocused on ${selectedNode.label} - recursively expanded to depth ${maxDepth}`);
  };
    ...
    <aside className="h-full bg-zinc-900/20 flex flex-col flex-shrink-0" style={{ width: `${detailsWidth}px` }}>
      ...
      >
        <div className="border-b border-zinc-800">
              ...
              {navigationPath.map((nodeId, index) => {
                ...
                return (
                  <Fragment key={nodeId}>
                    ...
                    {!isLast && <ChevronRight size={12} className="text-zinc-600" />}
                  </Fragment>
                );
              })}
            </div>
          ...
          {/* Layout selector */}
          <div className="px-4 py-2 flex items-center gap-2 border-t border-zinc-800/50">
            <span className="text-xs text-zinc-400">Layout:</span>
            <div className="flex gap-1">
              {['motion', 'graph', 'hierarchy', 'radial'].map((layout) => (
                <button
                  key={layout}
                  onClick={() => {
                    setGraphLayout(layout);
                    setStatus(`Switched to ${layout} layout`);
                  }}
                  className={clsx(
        ...
        <div className="flex-1 p-4 relative">
          {(graphLayout === 'motion' || graphLayout === 'graph') && (
            <ForceGraphView
              graphData={graphData}
              width={graphDims.w}
              height={graphDims.h}
              ...
              graphRef={graphRef}
              enablePhysics={graphLayout === 'motion'}
            />
          )}
              ...
              >
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex items-center justify-between pb-2 border-b border-zinc-800">
                    <span className="text-[10px] text-zinc-400">Force Simulation</span>
                    <button
                      onClick={() => {
                        const newValue = !graphSettings.enableForces;
                        setGraphSettings({ ...graphSettings, enableForces: newValue });
                        setStatus(newValue ? 'Force simulation enabled' : 'Force simulation disabled - static layout');
                      }}
                      className={`px-3 py-1 text-[10px] rounded transition ${
                        graphSettings.enableForces
                          ? 'bg-indigo-600 text-white'
                          : 'bg-zinc-800 text-zinc-400'
                      }`}
                    >
                      {graphSettings.enableForces ? 'ON' : 'OFF'}
                    </button>
                  </div>

                  {/* Charge Strength */}
                  <div className={!graphSettings.enableForces ? 'opacity-50 pointer-events-none' : ''}>
