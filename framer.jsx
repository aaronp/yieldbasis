...
const MESSAGES: Record<string, { id: string; from: string; text: string; dt: string }[]> = {
  alice: [
...
export type AccountId = "cash" | "family" | "savings";
export type TxType = "expense" | "income" | "transfer";

interface Account { id: AccountId; name: string; color: string; icon: "wallet" | "family" | "piggy" }
interface Tx { id: string; typ: TxType; amount: number; desc: string; from?: AccountId; to?: AccountId; dt: string; srcMessageId?: string }
...
function parseGBP(text: string): number | null {
  // Find first £amount in the string
  const m = text.replace(/,/g, "").match(/£\s*(\d+(?:\.\d{1,2})?)/i);
  return m ? Number(m[1]) : null;
}
function formatGBP(n: number) { return n.toLocaleString("en-GB", { style: "currency", currency: "GBP" }); }

// Tailwind bg class -> computed color (cached)
const __twBgCache = new Map<string, string>();
function getTailwindBgColor(twBgClass: string | undefined | null): string | null {
...
// --- Component ---------------------------------------------------------------
export default function ImmersiveChatPrototype() {
  ...
  const [accounts, setAccounts] = useState<Record<AccountId, number>>({ cash: 500, family: 1200, savings: 3000 });
  const [txs, setTxs] = useState<Tx[]>([]);

  // Refs to message bubbles & account chips for animation
  const msgRefs = useRef<Record<string, HTMLElement | null>>({});
  const acctRefs = useRef<Record<AccountId, HTMLElement | null>>({ cash: null, family: null, savings: null });

  // Floating tokens (coin animations)
  const [coins, setCoins] = useState<Array<{ id: string; x: number; y: number; amount: number }>>([]);
  ...
  const [graphDims, setGraphDims] = useState<{ w: number; h: number }>({ w: 640, h: 360 });
  useEffect(() => {
  ...
  // --- Budget actions --------------------------------------------------------
  function addTxFromMessage(msgId: string) {
    const msg = activeMessages.find((m) => m.id === msgId);
    if (!msg) return;
    const amt = parseGBP(msg.text);
    // Simple heuristic: "paid/bought" => expense from cash to family; "received" => income to cash from family
    const lower = msg.text.toLowerCase();
    const isExpense = /(paid|bought|spent)/.test(lower);
    const isIncome = /(received|got paid|salary|income)/.test(lower);
    const amount = amt ?? 20; // default demo amount if not in text
    ...
    // Start coin animation from message bubble -> target account
    ...
    if (srcEl && dstEl) {
      ...
      // animate via CSS transforms with Framer Motion, then commit balances
      // balances update slightly delayed to sync with arrival
      setTimeout(() => {
        setAccounts((a) => {
          const next = { ...a };
        ...
        // remove coin after arrival
        setCoins((cs) => cs.filter((c) => c.id !== id));
      }, 650);
    } else {
      // No animation targets, just apply
      commitBalances(tx);
    }
    setTxs((xs) => [tx, ...xs]);
  }

  function commitBalances(tx: Tx) {
    setAccounts((a) => {
      const next = { ...a };
          ...
          <div className="h-full grid grid-rows-[auto_1fr_auto]">
            ...
            <div className="overflow-y-auto p-4 space-y-3 bg-gradient-to-b from-zinc-950 to-zinc-900/30 relative">
              ...
              <div className="pointer-events-none absolute inset-0 z-10">
                {coins.map((c) => (
                  <Coin key={c.id} id={c.id} amount={c.amount} fromEl={msgRefs.current[txs.find(t=>t.id===c.id)?.srcMessageId ?? ""]} toEl={acctRefs.current[(txs.find(t=>t.id===c.id)?.typ === 'income' ? (txs.find(t=>t.id===c.id)?.to as AccountId) : (txs.find(t=>t.id===c.id)?.from as AccountId)) || 'cash']} viewport={viewport} />
                ))}
              </div>
            </div>
                  ...
                  <motion.div layoutId={`contact-${focusContactId}`} className="rounded-3xl border border-zinc-700/70 bg-zinc-900/70 shadow-2xl overflow-hidden">
                    ...
                    <div className="p-0">
                      ...
                      <div ref={graphContainerRef} className="px-5 pb-5">
                        <ForceGraph2D width={graphDims.w} height={graphDims.h} graphData={graphData} nodeLabel={(n: any) => n.label} nodeRelSize={6} cooldownTicks={40}
                          onEngineStop={() => {}}
                          linkColor={() => "#666"}
                          nodeCanvasObject={(node: any, ctx, globalScale) => {
                            const label = node.label as string; const fontSize = 12 / Math.sqrt(globalScale);
                          ...
                          onNodeClick={(n: any) => { if (n.kind === "group") { const id = String(n.id).replace("group:", ""); setPinnedGroupId(id); setFocusContactId(null); } }} />
                      </div>
...
function Coin({ id, amount, fromEl, toEl, viewport }: { id: string; amount: number; fromEl?: HTMLElement | null; toEl?: HTMLElement | null; viewport: {x:number;y:number} }) {
